...
namespace Tests
{
    ...
    public class Ex00Tests
    {
        ...
        public static IEnumerable<TestCaseData> MultiplyTests =>
            ...
            select new ArrayTestCaseData<float>(() => DataGenerator.GenerateFloatTestArray(stepSize, 66666 + i)).SetArgDisplayNames($"R{i}/{stepSize}");
        ...
        [TestCaseSource(nameof(MultiplyTests))]
        public unsafe void TestVectorizedUnrolled(FloatGenerator generator)
        {
            var (data, stepSize, reproContext) = generator();
            var copy = data.ToArray();
            fixed (float* dataPtr = &copy[0]) {
                Multiply.MultiplyVectorizedUnrolled(dataPtr, copy.Length, 1 / stepSize);
            }
