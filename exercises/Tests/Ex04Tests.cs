...
namespace Tests
{
    ...
    public class Ex04Tests
    {
        ...
        public void TestScalar(FloatGenerator generator)
        {
            var (data, stepSize, reproContext) = generator();
            var (exp, ints) = Decimalize.DecimalizeScalar(data);
            var divisor = 1.0f / Math.Pow(10, exp);
        ...
        [TestCaseSource(nameof(DecimalizeTests))]
        public void TestVectorized(FloatGenerator generator)
        {
        }
    }
}